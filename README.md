...
Workflow development is of course handled through the webinterface. Created workflows can be exported by grabbing their ID's, and then running `n8n export:workflow --id=<id> --output=<filename>.json`. In a very similar way, you can also import workflows already in the repository using `n8n import:workflow --input=<filename>.json`
